from tools import boll_macd_ga_tools_3
import pandas as pd
import time
...
def get_signal(df, params):
    ...
    boll_macd_ga_tools_3._get_boll(df, stock, boll_A, boll_std_len)
    boll_macd_ga_tools_3._get_macd(df, stock, macd_fast_len, macd_slow_len)
    ...
    df = boll_macd_ga_tools_3.public_func(df, 'long', 'signal1')
    ...
    df = boll_macd_ga_tools_3.public_func(df, 'short', 'signal2')
    ...
    df = boll_macd_ga_tools_3.public_func(df, 'long', 'signal3')
    ...
    df = boll_macd_ga_tools_3.public_func(df, 'short', 'signal4')
...
if __name__ == '__main__':
    ...
    pops = boll_macd_ga_tools_3.init_pops(pop_size, chromosome_length)
    iter = 15
    pc = 0.6  # 杂交概率
    pm = 0.01  # 变异概率
    ...
    while i < len(ori_df):
        ...
        best_fits = []
        for j in range(iter):
            obj_values = boll_macd_ga_tools_3.cal_fitness(test_df, pops, pop_size, chromosome_length)  # 计算绩效
            fit_values = boll_macd_ga_tools_3.clear_fit_values(obj_values)
            best_individual, best_fit = boll_macd_ga_tools_3.find_best(pops, fit_values,
                                                                       chromosome_length)  # 第一个是最优基因序列, 第二个是对应的最佳个体适度
            best_individuals.append(best_individual)
            best_fits.append(best_fit)
            results.append([best_individual, best_fit])
            boll_macd_ga_tools_3.selection(pops, fit_values)  # 选择
            boll_macd_ga_tools_3.crossover(pops, pc)  # 染色体交叉（最优个体之间进行0、1互换）
            boll_macd_ga_tools_3.mutation(pops, pm)  # 染色体变异（其实就是随机进行0、1取反
