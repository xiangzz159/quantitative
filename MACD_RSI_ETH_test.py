...
filenames = ['ETH2017-09-01-now-1H', 'ETH2017-09-01-now-2H', 'ETH2017-09-01-now-4H', 'ETH2017-09-01-now-1D']
l = []
for filename in filenames:
    for hist_ema in range(3, 12):
        for hist_signal_ma in range(3, 10):
            for hist_signal_ma_ in range(3, 10):
                for ii in floatrange(-5.0, 5.0, 0.005):
                    for jj in floatrange(-5.0, 5.0, 0.005):
                        money = 10000
                        ...
                        print(filename, hist_ema, hist_signal_ma, hist_signal_ma_, ii, jj, max_drawdown, max_money, round(true_times / total_times, 3), money)
                        l.append([filename, hist_ema, hist_signal_ma, hist_signal_ma_, ii, jj, max_drawdown, max_money, round(true_times / total_times, 3), money])

result = pd.DataFrame(l, columns=['filename', 'hist_ema', 'hist_signal_ma', 'hist_signal_ma_', 'i', 'j', 'max_drawdown', 'max_money', 'shooting', 'last_money'])
result.to_csv('ETH_result.csv')